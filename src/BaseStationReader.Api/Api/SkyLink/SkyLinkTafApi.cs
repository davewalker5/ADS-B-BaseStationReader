...
namespace BaseStationReader.Api.SkyLink
{
    internal class SkyLinkTafApi : SkyLinkApiBase, ITafApi
    {
            ...
            ExternalApiSettings settings) : base(client, factory)
        {
            ...
            var url = definition.ApiEndpoints.FirstOrDefault(x => x.EndpointType == ApiEndpointType.TAF)?.Url;
            _baseAddress = $"{url}";
        ...
        {
            Factory.Logger.LogMessage(Severity.Info, $"Looking up weather forecast for airport with ICAO code {icao}");
            ...
            string taf = GetStringValue(report, "raw");
            Factory.Logger.LogMessage(Severity.Debug, $"TAF for {icao} : {taf}");

            return string.IsNullOrEmpty(taf) ? null : [taf];
        }
    }
}